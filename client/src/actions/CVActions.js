...
export const suggestCareerObjective =
  (jobTitle, industry) => async (dispatch) => {
    try {
      const token = localStorage.getItem("userToken");
      if (!token) {
        throw new Error("Vui lòng đăng nhập trước");
      ...
      dispatch(
        cvSuggestObjectiveFail(
          error.response?.data?.message || "Lỗi khi gợi ý mục tiêu nghề nghiệp"
        )
      );
      throw error;
    }
...
export const generateWorkExperience = (experience) => async (dispatch) => {
  try {
    ...
    dispatch(
      cvGenerateExperienceFail(
        error.response?.data?.message || "Lỗi khi tạo kinh nghiệm"
      )
    );
    throw error;
  }
...
export const generatePDF = (data) => async (dispatch) => {
  try {
    ...
    const errorMessage =
      error.response?.data?.error || error.message || "Lỗi khi tạo PDF";
